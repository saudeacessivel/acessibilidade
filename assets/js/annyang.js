          ...
          for (var j = 0, l = commandsList.length; j < l; j++) {
            ...
            if (result) {
              ...
              if (parameters.length) {
                mensagem(result.input, parameters);
                  if (debugState) {
                    root.console.log('with parameters', parameters);
                  }
              }else{
                mensagem(result.input, null);
              }
 ...
 */
 ...
 var busca = function(tag) {
  console.log(tag);
  var busca_limpa = retira_acentos_voz(tag);
  ...
  if($('form').data('contraste') == 1 || $('#contraste-ativado').length){
    ...
    setTimeout(function() {
        window.location.href = contraste+'/'+busca_limpa;
     }, 5000);
  };
  ...
  var showConteudo = function(){
    ...
    }else if($('#acessibilidade').length){
      $('html,body').animate({
        scrollTop: ($('#acessibilidade').offset().top - 67),
      },
      { 
        duration: 1000, 
        complete: function(){
          $('#acessibilidade').find('h4 a:first').focus();
        }
      });
    }
  }
  ...
  var contraste = function() {
    ...
    setTimeout(function() {
        window.location.href = '/contraste';
     }, 5000);
  }

  var baixoContraste = function() {
    setTimeout(function() {
  ...
  var acessibilidade = function() {
     if($('form').data('contraste') == 1 || $('#contraste-ativado').length){
       var contratado = '/contraste';
     }else{
       var contratado = '';
     }
     
     setTimeout(function() {
        window.location.href = contratado+'/acessibilidade';
     }, 5000);
    //$(window.document.location).delay(3000).attr('href', );
  }

  var aumentarFonte = function(){
    var tamMaximo = 22;
    var tamMinimo = 10;
    var ajuste = 2;
  ...
  var tamanho = function(tag){
    $('body').css('font-size', parseInt(tag));
  }

  var commands = {
    'buscar posto em *search':  busca,
    'buscar posto na *search':  busca,
    'buscar posto no *search':  busca,
    'posto em *search':  busca,
    'posto na *search':  busca,
    'posto no *search':  busca,
    'estou em *search':  busca,
    'estou no *search':  busca,
    'estou na *search':  busca,
    'conteúdo':  showConteudo,
    'alto contraste':  contraste,
    'baixo contraste': baixoContraste,
    'acessibilidade': acessibilidade,
    'aumentar * texto': aumentarFonte,
    'diminuir * texto': diminuirFonte,
    'texto *tamanho': tamanho/*,
    'aumentar fonte':  showConteudo,
    'diminuir fonte': showConteudo,*/
  };
...
var mensagem = function(paremetros, tags){
  $('#mensagem_comando p').html('<a href="#mensagem_comando"><strong>Comando de Voz Ativado</strong><br/> "'+paremetros+'"</a>');
  $('#mensagem_comando p').focus();
  $('#mensagem_comando').fadeIn(500).delay(2000).fadeOut(500);
}